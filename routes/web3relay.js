...
// load config.json
const config = { nodeAddr: 'localhost', gethPort: 8545 };
try {
...
// load token list
const KnownTokenDecimalDivisors = {};
const KnownTokenInfo = {};
...
exports.data = async (req, res) => {
  if ('tx' in req.body) {
    ...
    Transaction.findOne({ hash: txHash }).lean(true).exec(async (err, doc) => {
      ...
      if (!transactionResponse.blockNumber) {
    ...
    asyncL.waterfall([
      function (callback) {
        ...
        web3.trace.transaction(txHash, (err, traces) => {
          if (err || !traces) {
        ...
        // detect some known contracts
          const methodSig = tx.input.substr(0, 10);
          if (KnownMethodIDs[methodSig]) {
          // is it a token contract ?
            if (KnownMethodIDs[methodSig].type === 'ERC20') {
              const contract = web3.eth.contract(ERC20ABI);
              const token = contract.at(tx.to);
      ...
      }], (error, tx, traces, contractOrDb, tokenContract) => {
      if (error) {
        if (error === true) error = { error: true, message: 'normal TX' };
    ...
    const addr = req.body.addr_trace.toLowerCase();
    ...
    asyncL.waterfall([
      function (callback) {
        ...
        Transaction.findOne({ creates: addr }).lean(true).exec((err, doc) => {
          if (err || !doc) {
      ...
      function (transaction, callback) {
        ...
        if (transaction) {
          const bytecode = web3Addr.eth.getCode(addr);
          if (bytecode.length > 2) {
            const contract = web3Addr.eth.contract(ERC20ABI);
            const token = contract.at(addr);
    ...
    ], (error, transaction, token, decimals, lastBlockNumber, callback) => {
      // check divisor
      ...
      const filter = { 'fromBlock': web3.utils.toHex(fromBlock), 'toAddress': toAddr };
          ...
          if (!transaction) {
            ...
            txns.forEach((t) => {
              if (t.type === 'call') {
                ...
                const toAddr = callInfo && callInfo.params && callInfo.params[0].value;
            ...
            res.write(JSON.stringify({
              transactions: transfers, createTransaction: transaction, after, count: filter.count,
            }));
          } else {
