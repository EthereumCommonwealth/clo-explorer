...
module.exports = function (req, res) {
  ...
  else if (req.body.action === 'miners') getMinerStats(req, res);

  else if (req.body.action === 'hashrate') getHashrate(res);

  else if (req.body.action === 'hashrates') getHashrates(req, res);

  else if (req.body.action === 'txns') getTxStats(req, res);
...
**/
const getMinerStats = function (req, res) {
 ...
 */
const getTxStats = function (req, res) {
  const days = config.settings.stats && config.settings.stats.txnDays || 30;
  let range = 24 * days * 60 * 60;
  // check validity of range
  if (req.body.range && req.body.range < 60 * 60 * 24 * 7) {
    range = parseInt(req.body.range);
  ...
  // select mod
  const rngs = [60 * 60, 2 * 60 * 60, 4 * 60 * 60, 6 * 60 * 60, 12 * 60 * 60,
    24 * 60 * 60, 7 * 24 * 60 * 60, 14 * 24 * 60 * 60, 30 * 24 * 60 * 60, 60 * 24 * 60 * 60,
  ];
  const mods = [30 * 60, 30 * 60, 60 * 60, 60 * 60, 60 * 60,
  ...
  let timebefore = parseInt((new Date()).getTime() / 1000) - range;
  ...
  }]).sort('timestamp').exec((err, result) => {
    if (err || !result) {
      console.error(err);
      res.status(500).send();
    } else {
...
**/
const getHashrates = function (req, res) {
  // setup default range
...
**/
const getHashrate = function (res) {
