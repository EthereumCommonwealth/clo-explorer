...
const grabBlocks = function (config) {
  if ('listenOnly' in config && config.listenOnly === true) listenBlocks(config, web3);
  else {
    setTimeout(() => {
...
var listenBlocks = function (config, web3) {
  const newBlocks = web3.eth.filter('latest');
...
var grabBlock = function (config, web3, blockHashOrNumber) {
  ...
  if (web3.isConnected()) {
    web3.eth.getBlock(desiredBlockHashOrNumber, true, (error, blockData) => {
      if (error) {
        ...
        if (!('skipTransactions' in config && config.skipTransactions === true)) writeTransactionsToDB(config, blockData);
        if ('listenOnly' in config && config.listenOnly === true) return;
...
var writeBlockToDB = function (config, blockData) {
  return new Block(blockData).save((err, block, count) => {
...
var checkBlockDBExistsThenWrite = function (config, blockData) {
  Block.find({ number: blockData.number }, (err, b) => {
    if (!b.length) writeBlockToDB(config, blockData);
    else {
...
var writeTransactionsToDB = function (config, blockData) {
  ...
  if (blockData.transactions.length > 0) {
    ...
    Transaction.collection.insert(bulkOps, (err, tx) => {
      if (typeof err !== 'undefined' && err) {
        if (err.code == 11000) {
          console.log(`Skip: Duplicate key ${
            err}`);
...
const web3 = new Web3(new Web3.providers.HttpProvider(`http://${config.nodeAddr}:${config.gethPort.toString()}`));
